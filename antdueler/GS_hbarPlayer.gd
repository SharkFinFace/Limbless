...
func _process(_delta):
	...
	rect_position = Vector2(OS.window_size.x-rect_size.x,0)
	
	if scorekeeper.healthPlayer == 4:
		yield(get_tree().create_timer(1.0), "timeout")
		get_node("AnimationPlayer").play("four")
	elif scorekeeper.healthPlayer == 3:
		yield(get_tree().create_timer(1.0), "timeout")
		get_node("AnimationPlayer").play("three")
	elif scorekeeper.healthPlayer == 2:
		yield(get_tree().create_timer(1.0), "timeout")
		get_node("AnimationPlayer").play("two")
	elif scorekeeper.healthPlayer == 1:
		yield(get_tree().create_timer(1.0), "timeout")
		get_node("AnimationPlayer").play("one")
	elif scorekeeper.healthPlayer == 0:
		yield(get_tree().create_timer(1.0), "timeout")
