...
class_name InteractableEscalatorOne
...
export (Vector3) var newPosition
export (Vector2) var newDirection
...
#	bmasGetOut.playing = true
	...
	playerNode.target_yaw_pitch = newDirection
	playerNode.translation = newPosition
	yield(get_tree().create_timer(duration - 1.05), "timeout")	
...
#	get_node("/root/Spatial/EscalatorOne").queue_free()
